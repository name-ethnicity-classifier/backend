...
"""
@model_routes.route("/classify", methods=["POST"])
@jwt_required()
def classify_names():
    ...
    try:
        ...
        # Validate request body
        request_data = ClassificationSchema(**request.json)

        # TODO classify

        # TODO Classify
        current_app.logger.info("Successfully classified names.")
        return success_response(message="Successfully classified names.")


    # Handle Pydantic schema validation errors
    except ValidationError as e:
        current_app.logger.error("Clasification data validation failed. Error:\n{e}")
        return error_response(
            error_code="INVALID_CLASSIFICATION_DATA", message="Invalid classification request data.", status_code=422
        )
    ...
    # Handle SQLAlchemy errors
        ...
        current_app.logger.error(f"Failed fetch data from database. Error:\n{e}")
        return error_response(
            error_code="SERVER_SIDE_ERROR", message="Failed to receive user data.", status_code=500
        )

    # Handle unexpected errors
    except Exception as e:
        current_app.logger.error(f"Unexpected error while processing classification request data. Error:\n{e}")
        return error_response(
            error_code="UNEXPECTED_ERROR", message="Unexpected error.", status_code=500
        )
"""